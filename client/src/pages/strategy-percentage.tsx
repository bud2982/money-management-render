...
export default function StrategyPercentage() {
  ...
  useEffect(() => {
    // Imposta un valore globale di bankroll se non esiste
    const globalBankroll = Number(localStorage.getItem('globalBankroll') || '10000');
    if (!localStorage.getItem('globalBankroll')) {
      localStorage.setItem('globalBankroll', '10000');
    }
    
    const allocatedBankroll = Number(localStorage.getItem('allocatedBankroll') || '0');
    
    // Assicura che ci sia sempre un bankroll disponibile
    let availableBankroll = Number(localStorage.getItem('availableBankroll') || '0');
    if (availableBankroll <= 0) {
      // Se non c'è bankroll disponibile, reimposta a 5000
      availableBankroll = 5000;
      localStorage.setItem('availableBankroll', availableBankroll.toString());
      console.log("Bankroll disponibile reimpostato a 5000");
    }
    ...
    console.log("Bankroll disponibile:", availableBankroll);
  }, []);
        ...
        <>
          <Card className="mb-6">
            <CardContent className="p-6 pb-4">
              ...
              {/* Sezione per nuova scommessa - sempre visibile */}
